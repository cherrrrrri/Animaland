...
public class UndergroundStudyRoom extends Activity {
    ...
    private static boolean first_underground=false;
    private Handler handler1 = new Handler(Looper.myLooper()){
        @Override
        public void handleMessage(@NonNull Message msg) {
            super.handleMessage(msg);
            if(msg.what == 0){
                Intent intent = new Intent(UndergroundStudyRoom.this, selfRoomMainActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_underground_study_room);


        //找到控件
        mRecycleView = findViewById(R.id.rv_underground);
        mscrollView = findViewById(R.id.sc_under);
        mCreate=findViewById(R.id.room_create_UNDER);
        mUp =findViewById(R.id.room_down_UNDER);
        find_room = findViewById(R.id.room_search);//寻找房间的文本框
        check=findViewById(R.id.check_cave);
        mBack=findViewById(R.id.room_back);

        if(first_underground==false) {
            NewbieGuide.with(this)
                    .setLabel("guide1")
                    .alwaysShow(true)
                    .addGuidePage(GuidePage.newInstance()
                            .setLayoutRes(R.layout.cave_guide))
                    .addGuidePage(GuidePage.newInstance()
                            .setLayoutRes(R.layout.cave_guide2))
                    .addGuidePage(GuidePage.newInstance()
                            .addHighLight(mUp)
                            .setLayoutRes(R.layout.cave_guide3))
                    .show();
            first_underground=true;
        }
        ...
        mBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent=new Intent(UndergroundStudyRoom.this, Map.class);
                startActivity(intent);
            }
        });
            ...
            public void onClick(View view) {
                ...
                Runnable runnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        dialog_roomcreate roomcreate = new dialog_roomcreate(UndergroundStudyRoom.this, R.style.dialogOfShowRoom);
                            ...
                            public void onCreateRoom(dialog_roomcreate roomcreate) throws SQLException, ClassNotFoundException, IllegalAccessException, InstantiationException {
                                ...
                                if (u.checkRoomName(room_name))//如果房间名字符合要求
                                {
                                    if ( u.checkRoomPwd(room_pwd)) {//存入数据库:房间密码正确
                                        try {
                                            db.createUndergroundRoom(room_name, room_pwd);
                                            Intent intent = new Intent(UndergroundStudyRoom.this, selfRoomMainActivity.class);
                                            startActivity(intent);
                                        ...
                                        Toast.makeText(UndergroundStudyRoom.this, "请输入四位密码（只包含英文大小写或数字）", Toast.LENGTH_SHORT).show();
                                    }
                                } else {
                                    Toast.makeText(UndergroundStudyRoom.this, "房间名必须在2-10个字符之间，且都要为中英文或数字", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
                        roomcreate.show();
...
//thread.start();
                ...
                Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (find.isEmpty()) {//如果搜索框为空
                            Toast.makeText(UndergroundStudyRoom.this, "请输入房间id", Toast.LENGTH_SHORT).show();
                        } else {
                            if ((find.length() == 5) && isNumberic(find)) {//如果长度为5且都为数字:可继续
                                try {
                                    if (db.findRoom(find)) {//如果在数据库找到了id
                                        if (!db.isFull(find)) {//如果人数没满
                                            //db.enterRoom(find);
                                            dialog_roomshow roomshow = new dialog_roomshow(UndergroundStudyRoom.this, R.style.dialogOfShowRoom);
                                                        ...
                                                        public void onEnterRoom(dialog_roomshow roomshow) {//公告
                                                            ...
                                                            try {
                                                                if(!db.hasPwd(find)){//如果没密码 直接进入房间
                                                                    ...
                                                                    dialog_roomeditpass roomeditpass = new dialog_roomeditpass(UndergroundStudyRoom.this, R.style.dialogOfShowRoom);
                                                                            ...
                                                                            if(u.checkRoomPwd(roomPwd)){//密码格式正确
                                                                                try {
                                                                                    if(db.pwdFound(find,roomPwd)){//如果房间名和密码匹配
                                                                                        ...
                                                                                        Toast.makeText(UndergroundStudyRoom.this,"密码错误，请重新输入",Toast.LENGTH_SHORT).show();
                                                                                    }
                                                                                } catch (SQLException e) {
                                                                                    e.printStackTrace();
                                                                                } catch (ClassNotFoundException e) {
                                            ...
                                            Toast.makeText(UndergroundStudyRoom.this, "房间人数已满", Toast.LENGTH_SHORT).show();
                                        }
                                    } else {
                                        Toast.makeText(UndergroundStudyRoom.this, "该房间不存在", Toast.LENGTH_SHORT).show();
                                    }
                                } catch (SQLException e) {
                                    e.printStackTrace();
                                } catch (ClassNotFoundException e) {
                                ...
                                Toast.makeText(UndergroundStudyRoom.this, "请输入正确房间id", Toast.LENGTH_SHORT).show();
                            }
                        }
                        Looper.loop();
                    }
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    for (int i = 0; i < db.existUndergroundRoomNumber(); i++) {//房间数量
                        room roomData = new room();
                        roomData.id = db.existUndergroundRoom().get(i);//房间id
                        roomData.roomName = db.findRoomName(roomData.id);
                        roomData.roomMember = db.findMemberNumber(roomData.id);
                        roomData.icon=roomPictures.roomPics[i%3];//加载图片
                        switch (i%3){
                            case 0:
                                roomData.type=1;
                                break;
                    ...
                    public void run() {
                        try {
                            ...
                            if (!db.isFull(id)) {//如果人数没满
                                ...
                                Toast.makeText(UndergroundStudyRoom.this,"房间已满",Toast.LENGTH_SHORT).show();
                            }
                            Looper.loop();
                        } catch (SQLException e) {
                            e.printStackTrace();
    ...
    private void showStagger(boolean isVertical, boolean isReverse) {
        ...
        //创建适配器Adapter
        madapter = new MoreTypeBeanAdapter(mroomList,RecycleViewBaseAdapter.THEME_CAVE);
